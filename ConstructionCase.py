# Construction Case   v3.0  (c) Adi Andrei / Radu Andrei / Technosophics Ltd 1989 - 2022 - ALL RIGHTS RESERVED
...
def subgrade_layer():
    ...
    if (C==12 or C==13 or C==14 or C==16 or C==17 or C==18 or C==19 or C==23 or C==24 or C==25) \
            and CP==1 and SA==1 and RU==1 : K=29
    if (C==12 or C==13 or C==14 or C==16 or C==17 or C==18 or C==19 or C==23 or C==24 or C==25) \
            and CP==1 and SA==0 and (RU==1 or RU==0) : K=30
    if (C==12 or C==13 or C==14 or C==16 or C==17 or C==18 or C==19 or C==23 or C==24 or C==25) \
            and CP==0 and SA==0 and RU==0 : K=31
...
##### Main  Program #####
def main():
    print("\n\nConstruction Case v3.0  (c) Adi Andrei / Radu Andrei / Technosophics Ltd 1989 - 2022")

    global  WN, WO, CBR, IC, DM, F, EN, A, G, IP, FRG, DU, C, K, CP, SA, RU

    print("\nEnter Data :")
    WN = Decimal(input("natural humidity (WN)="))
    WO = Decimal(input("optimum humidity (WO)="))
    CBR = Decimal(input("bearing capacity index (CBR)="))
    IC = Decimal(input("consistency index (IC)="))
    DM = Decimal(input("maximum size (DM)="))
    F = Decimal(input("fine fractions (F)="))
    EN = Decimal(input("sand equivalent (EN)="))
    A = Decimal(input("alterability (A)="))
    G = Decimal(input("gross fractions (G)="))
    ...
    if C == 0:
        ...
        if input("\nWould you like the subgrade-layer construction case identification too?  (y/n)") == 'y':
            CP = Decimal(input("bearing capacity (CP)="))
            SA = Decimal(input("sensitivity to water action (SA)="))
            RU = Decimal(input("risk of wetting from underground waters  (RU)="))

            K = subgrade_layer()
            if K == 0:
                print("*** Sorry - Unidentified subgrade-layer construction case ***")
            else:
                print("### Subgrade-layer construction case identified nr:", K)
